...
// services - contains list of Descriptor
var services []*Descriptor

// RegisterService - add instance's discriptor into services
...
// Register - add descriptor into services
func Register(descriptor *Descriptor) {
	services = append(services, descriptor)
}

// GetServices - return services with sorted priority
...
// are expected to forfill to be started.
type Service interface {

	// Init is called by main process which gives the service
...
type BackgroundService interface {
	...
	// to subscribe to ctx.Done() so the service can be notified when shuts down.
